...
\begin{frame}
    In the previous section, we reweighted observation to be able to re-run the original algorithm, this can also be done through random projection, to reduce the computation cost of finding the exact solution to OLS require \(\mathcal{O}(nd^2)\) time.
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item \alert{\textbf{Finite OLS:}} We then run standard OLS on this sketch to get our predictions: \(\hat{y} = \hat{\Phi}(\hat{\Phi}^\top \hat{\Phi})^{-1}\hat{\Phi}^\top y\). Crucially, we only have to invert the much smaller \(s \times s\) covariance matrix \((\hat{\Phi}^\top \hat{\Phi})\).
    \end{itemize}
