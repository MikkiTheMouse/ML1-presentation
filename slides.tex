...
\begin{frame}{Index of the presentation: part I}
    \tableofcontents[part=1]
\end{frame}
\begin{frame}{Index of the presentation: part II}
    \tableofcontents[part=2]
\end{frame}
\begin{frame}{Index of the presentation: part III}
...
\subsection{For linear estimators}
\begin{frame}
...
\begin{frame}
    ...
    \[
    \hat{f}(x)=\sum\limits_{i=1}^nV_iy_{(i)}(x).\footnote{\text{Here we mean \(\hat{f}(x)=\frac{1}{m}\sum_{b=1}^m\hat{f}^{(b)}(x)\).}}
    \]
...
\begin{frame}{}{Example: one-nearest neighbor regression. Variance}
    For the \example{\textbf{variance}} term, since (using \(y_{(i)}=f_\star(x_{(i)})+\varepsilon_i\)):
    \[
        \text{var}(\hat{f}(x)) = \text{var}\left( \sum_{i=1}^n V_i y_{(i)} \right)=\sum_{i=1}^n \text{var}(V_i y_{(i)})=\sum_{i=1}^n V_i^2 \text{var}(y_{(i)})=\sigma^2 \sum_{i=1}^n V_i^2,
    \]
    (where \(\sigma^2\) represents the variance of the irreducible random noise) we need a bound for \(\sum_{i=1}^n V_i^2\).

    Combinatorically, it can be shown that\footnote{See section 10.1.2, p. 286 \textit{Learning Theory from First Principles}, by \textit{Francis Bach}.}: \(V_i\leq\frac{s}{n-s+1}\prod\limits_{j=0}^{s-2}\left(1-\frac{i}{n}\right)\), which leads to \(\sum\limits_{i=1}^nV_i^2\leq\frac{s}{n}\frac{1}{(1+1/n-s/n)^2}\).
\end{frame}

\begin{frame}{}{Example: one-nearest neighbor regression. Bias}
    For the \example{\textbf{bias}} term, since (using again \(y_{(i)}=f_\star(x_{(i)})+\varepsilon_i\)):
    \[
        \mathbb{E}[\hat{f}(x)] - f_\star(x)=\sum_{i=1}^n V_i f_\star(x_{(i)}) - f_\star(x)=\sum_{i=1}^n V_i \left( f_\star(x_{(i)}) - f_\star(x) \right),
    \]
    assuming \(f_\star\) to be \(B\)-Lipschitz continuous (meaning \(|f_\star(x_{(i)}) - f_\star(x)| \leq B \|x - x_{(i)}\|\)), by applying the Lipschitz condition and squaring the result we get:
    \[
        (\mathbb{E}[\hat{f}(x)] - f_\star(x))^2\leq B^2 \sum_{i=1}^n V_i \mathbb{E}\left[\|x - x_{(i)}\|^2\right].
    \]
    Since the right term is \(B^2\) multiplied by the expectation of the distance to the first neighbor from a random sample of size \(s\):
    \[
        B^2 \sum_{i=1}^n V_i \mathbb{E}\left[\|x - x_{(i)}\|^2\right]\leq 4B^2\text{diam}(\mathcal{X})^2\frac{1}{s^{2/d}},\footnote{\text{See lemma 6.1, p. 168 \textit{Learning Theory from First Principles}, by \textit{Francis Bach}.}}
    \]
    where \(\mathcal{X}\) is the subset of \(\mathbb{R}^d\) where the input data live.
\end{frame}

\begin{frame}{}{Example: one-nearest neighbor regression}
